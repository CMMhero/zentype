...
/**
 * Base UI only resolves labels for `<Select.Value>` from the root `items`
 * data structure — labels on child `<SelectItem>`s are never registered, so
 * without it the trigger renders the raw value. Derive that structure from
 * the rendered items (single source of truth) so the trigger always shows
 * the option label. Callers can still pass an explicit `items` map.
 */
function extractItemLabel(node: React.ReactNode): string | undefined {
  if (typeof node === "string") return node;
  if (typeof node === "number") return String(node);
  if (Array.isArray(node)) {
    const parts: string[] = [];
    for (const item of node) {
      const text = extractItemLabel(item);
      if (text !== undefined) parts.push(text);
    }
    return parts.length > 0 ? parts.join("") : undefined;
  }
  if (React.isValidElement(node)) {
    // Descend into fragments; non-text children (icons, etc.) are ignored.
    const props = node.props as { children?: React.ReactNode };
    if (node.type === React.Fragment) {
      return extractItemLabel(props.children);
    }
    return undefined;
  }
  return undefined;
...
function collectItemLabels(node: React.ReactNode, map: Record<string, string>): void {
  React.Children.forEach(node, (child) => {
    ...
    if (child.type === SelectItem) {
      ...
      const labelText = typeof label === "string" ? label : extractItemLabel(props.children);
      if (labelText !== undefined) map[value] = labelText;
      return;
    }
