            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isCurrentUser && "border-primary/50 bg-primary/5 rounded-2xl border-2",
                  showCrown &&
                    ranking.rank === 1 &&
                    "bg-gradient-to-r from-yellow-500/10 via-amber-500/5 to-transparent",
                  showCrown &&
                    ranking.rank === 2 &&
                    "bg-gradient-to-r from-gray-300/10 via-gray-400/5 to-transparent",
                  showCrown &&
                    ranking.rank === 3 &&
                    "bg-gradient-to-r from-orange-400/10 via-orange-500/5 to-transparent",
                  onUserClick && "hover:bg-muted/40 cursor-pointer",
                )}
              >
                <div className="flex w-12 items-center gap-1">
                  <span className="w-4 text-sm font-semibold tabular-nums">{ranking.rank}</span>
                  {showCrown ? (
                    <IconCrown className={cn("size-5", crownColor)} aria-hidden="true" />
                  ) : null}
                </div>

                <Avatar className="size-10">
                  {ranking.avatarUrl && (
                    <AvatarImage src={ranking.avatarUrl} alt={`${displayName} avatar`} />
                  )}
