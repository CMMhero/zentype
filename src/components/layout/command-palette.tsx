...
import { TIME_OPTIONS, WORD_OPTIONS } from "~/lib/types";
  ...
  const items: { id: string; group: string; value: string; keywords: string; label: string }[] = [
    {
      id: "nav-test",
      group: "navigate",
      value: "navigate test alt+1",
      keywords: "navigate test",
      label: "test",
    },
    {
      id: "nav-lb",
      group: "navigate",
      value: "navigate leaderboard global bests alt+2",
      keywords: "navigate leaderboard",
      label: "leaderboard",
    },
    {
      id: "nav-profile",
      group: "navigate",
      value: "navigate profile dashboard alt+3",
      keywords: "navigate profile",
      label: "profile",
    },
    {
      id: "nav-settings",
      group: "navigate",
      value: "navigate settings config alt+4",
    ...
    ...TIME_OPTIONS.map((t) => ({
      ...
      keywords: `mode time ${t}s`,
      label: `${t}s time`,
    })),
    ...WORD_OPTIONS.map((w) => ({
      id: `mode-words-${w}`,
      group: "mode",
      value: `mode words ${w} words type ${w} words`,
      keywords: `mode words ${w}`,
      label: `${w} words`,
    })),
    ...THEMES.map((t) => ({
      id: `theme-${t.id}`,
    ...
    })),
    {
      ...
      keywords: "sound audio enable disable toggle on off",
      label: "sound toggle",
    },
    {
      id: "sound-error",
      ...
      keywords: "sound error beep toggle on off",
      label: "error sound",
    },
    ...
    })),
    {
      ...
      keywords: "gameplay blind mode toggle on off",
      label: "blind mode",
    },
    {
      id: "gp-stop",
      ...
      keywords: "gameplay stop error toggle on off",
      label: "stop on error",
    },
    {
      id: "gp-strict",
      ...
      keywords: "gameplay strict space toggle on off",
      label: "strict space",
    },
    {
      id: "gp-back",
      ...
      keywords: "gameplay free backspace toggle on off",
    ...
    },
    {
      ...
      keywords: "gameplay hide live stats toggle on off",
      label: "hide live stats",
    },
    {
      id: "gp-hide-progress",
      group: "gameplay",
      value: "gameplay hide progress time word count bar toggle on off",
      keywords: "gameplay hide progress toggle on off",
      label: "hide progress",
    },
    {
      id: "gp-punct",
      ...
      keywords: "gameplay punctuation toggle on off",
      label: "punctuation",
    },
    {
      id: "gp-nums",
      ...
      keywords: "gameplay numbers toggle on off",
      label: "numbers",
    },
    {
      id: "app-kb",
      ...
      keywords: "appearance keyboard toggle on off",
    ...
    },
    {
      ...
      keywords: "appearance smooth caret toggle on off",
      label: "smooth caret",
    },
    {
      id: "act-restore",
      group: "actions",
      value: "actions restore defaults reset settings",
      keywords: "actions restore defaults reset settings",
      label: "restore defaults",
    },
    {
      id: "act-export",
      group: "actions",
      value: "actions export json data download",
      keywords: "actions export json data download",
      label: "export json",
    },
    {
      ...
      keywords: "actions delete account profile data",
    ...
    },
    {
      ...
      keywords: "actions sign out log out logout",
      label: "sign out",
    },
    {
      id: "act-discard",
      group: "actions",
      value: "actions discard local guest results",
      keywords: "actions discard local guest results",
      label: "discard local results",
    },
    {
      id: "link-about",
      group: "links",
      value: "link about page info",
      keywords: "link about page info",
      label: "about",
    },
    {
      id: "link-terms",
      group: "links",
      value: "link terms of service tos",
      keywords: "link terms of service tos",
      label: "terms",
    },
    {
      id: "link-privacy",
      group: "links",
      value: "link privacy policy gdpr",
      keywords: "link privacy policy gdpr",
      label: "privacy",
    },
    {
      id: "link-github",
      group: "links",
      value: "link github source code repository",
      keywords: "link github source code repository",
      label: "github",
    },
  ];
  return items;
...
export function CommandPalette() {
  ...
  const toggleSound = useCallback(
    () => update({ sound: { ...settings.sound, enabled: !settings.sound.enabled } }),
    [update, settings.sound],
  );
  const toggleSoundError = useCallback(
    () => update({ soundOnError: !settings.soundOnError }),
    [update, settings.soundOnError],
  );
  const toggleBlind = useCallback(
    () => update({ blindMode: !settings.blindMode }),
    [update, settings.blindMode],
  );
  const toggleStop = useCallback(
    () => update({ stopOnError: !settings.stopOnError }),
    [update, settings.stopOnError],
  );
  const toggleStrict = useCallback(
    () => update({ strictSpace: !settings.strictSpace }),
    [update, settings.strictSpace],
  );
  const toggleFreeBack = useCallback(
    () => update({ freeBackspace: !settings.freeBackspace }),
    [update, settings.freeBackspace],
  );
  const toggleHideLive = useCallback(
    () => update({ hideLiveStats: !settings.hideLiveStats }),
    [update, settings.hideLiveStats],
  );
  const toggleHideProgress = useCallback(
    () => update({ hideProgress: !settings.hideProgress }),
    [update, settings.hideProgress],
  );
  const toggleKeyboard = useCallback(
    () => update({ showKeyboard: !settings.showKeyboard }),
    [update, settings.showKeyboard],
  );
  const toggleSmoothCaret = useCallback(
    () => update({ smoothCaret: !settings.smoothCaret }),
    [update, settings.smoothCaret],
  );
  const togglePunctuation = useCallback(
    () => update({ punctuation: !settings.punctuation }),
    [update, settings.punctuation],
  );
  const toggleNumbers = useCallback(
    () => update({ numbers: !settings.numbers }),
    [update, settings.numbers],
  );
  ...
  const exportJson = useCallback(() => {
    ...
    const payload = {
      ...
      version: 2,
      profile: user
        ? { username: user.username, email: user.email, providers: user.providers }
        : null,
      settings,
      localResults: local,
    };
  ...
  return (
    ...
    >
      <CommandInput
        placeholder="search users, settings, or commands…"
      ...
      <CommandList>
        ...
        <CommandGroup heading="navigate">
          <CommandItem
            ...
            onSelect={() => go("/")}
          >
            <IconKeyboard /> test<Shortcut>alt+1</Shortcut>
          </CommandItem>
          <CommandItem
            value="navigate leaderboard global bests alt+2"
            keywords={["navigate", "leaderboard"]}
          ...
          >
            <IconTrophy /> leaderboard<Shortcut>alt+2</Shortcut>
          </CommandItem>
          <CommandItem
            value="navigate profile dashboard alt+3"
            keywords={["navigate", "profile"]}
            onSelect={() => go("/profile")}
          >
            <IconUser /> profile<Shortcut>alt+3</Shortcut>
          </CommandItem>
          <CommandItem
            value="navigate settings config alt+4"
            keywords={["navigate", "settings"]}
          ...
          >
            <IconSettings /> settings<Shortcut>alt+4</Shortcut>
          </CommandItem>
        </CommandGroup>
        ...
        <CommandGroup heading="actions">
          <CommandItem
            value="actions restore defaults reset settings"
            keywords={["actions", "restore", "defaults", "reset"]}
            onSelect={restoreDefaults}
          >
            <IconRefresh /> restore defaults
            <CommandDesc>reset theme, font, sound, and gameplay</CommandDesc>
          </CommandItem>
          <CommandItem
            value="actions export json data download"
            keywords={["actions", "export", "json", "data", "download"]}
          ...
          {user && (
            <CommandItem
              value="actions delete account permanently remove profile data"
              keywords={["actions", "delete", "account", "profile", "data"]}
              onSelect={openDeleteAccount}
              className="text-destructive focus:text-destructive"
            >
              <IconAlertTriangle /> delete account
              <CommandDesc>permanently delete profile, results, and xp</CommandDesc>
            </CommandItem>
          )}
          {user && (
            <CommandItem
          ...
          {local.length > 0 && (
            <CommandItem
              value="actions discard local guest results"
              keywords={["actions", "discard", "local", "guest"]}
              onSelect={discardLocal}
              className="text-destructive focus:text-destructive"
            >
              <IconAlertTriangle /> discard local results
              <CommandDesc>
                delete {local.length} unsynced guest result{local.length === 1 ? "" : "s"}
              </CommandDesc>
            </CommandItem>
          )}
        </CommandGroup>
        ...
        <CommandGroup heading="links">
          ...
          <CommandItem
            value="link terms of service tos"
            keywords={["link", "terms", "service", "tos"]}
            onSelect={() => go("/terms")}
          >
          ...
          <CommandItem
            value="link github source code repository"
            keywords={["link", "github", "source", "code"]}
            onSelect={() => window.open("https://github.com/CMMhero/zentype", "_blank")}
          >
              ...
              {THEMES.map((t) => (
                ...
                >
                  ...
                  <span className="mr-1.5 inline-flex gap-0.5" aria-hidden>
                    ...
                    <span
                      className="size-3 rounded-sm"
                      style={{ background: t.vars["--secondary"] }}
                    />
                  </span>
                  {t.label}
                  <CommandDesc>{t.appearance} theme</CommandDesc>
                </CommandItem>
              ))}
            </CommandGroup>
          </>
        ...
        <CommandGroup heading="sound">
          ...
          >
            ...
            sound <ToggleBadge on={activeSet.soundEnabled} />
            <CommandDesc>enable or disable keystroke audio feedback</CommandDesc>
          </CommandItem>
          <CommandItem
            value="sound error beep incorrect character toggle on off"
          ...
          >
            <IconAlertTriangle /> error sound <ToggleBadge on={activeSet.soundOnError} />
            <CommandDesc>play a sound when you type an incorrect character</CommandDesc>
          </CommandItem>
          {SOUND_VARIANTS.map((v) => (
            <CommandItem
              key={v.value}
              ...
              keywords={["sound", v.label]}
              onSelect={() => {
                update({ sound: { ...settings.sound, variant: v.value } });
                playKeypress(v.value, settings.sound.volume);
              }}
              className={activeSet.soundVariant === v.value ? "text-primary bg-primary/10" : ""}
            >
              <IconMusic /> {v.label}
              <CommandDesc>{v.desc}</CommandDesc>
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        <CommandGroup heading="caret">
          {CARET_STYLES.map((c) => (
            <CommandItem
              ...
              keywords={["caret", c.label]}
              onSelect={() => {
                update({ caretStyle: c.value });
                close();
              }}
              className={activeSet.caret === c.value ? "text-primary bg-primary/10" : ""}
            >
              <IconCursorText /> {c.label}
              <CommandDesc>{c.desc}</CommandDesc>
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        <CommandGroup heading="visible lines">
          {([1, 2, 3] as const).map((n) => (
            <CommandItem
              ...
              onSelect={() => {
                update({ visibleLines: n });
                close();
              }}
              className={activeSet.visibleLines === n ? "text-primary bg-primary/10" : ""}
            >
              <IconList /> {n} line{n > 1 ? "s" : ""}
              <CommandDesc>
                show {n} line{n > 1 ? "s" : ""} of text at a time
              </CommandDesc>
            </CommandItem>
          ))}
        </CommandGroup>
        ...
        <CommandGroup heading="gameplay">
          ...
          >
            ...
            blind mode <ToggleBadge on={activeSet.blindMode} />
            <CommandDesc>hide error coloring</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay stop on error block cursor until fixed toggle on off"
          ...
          >
            <IconPlayerStop /> stop on error <ToggleBadge on={activeSet.stopOnError} />
            <CommandDesc>block cursor until fixed</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay strict space wrong words cannot be skipped toggle on off"
          ...
          >
            <IconSpace /> strict space <ToggleBadge on={activeSet.strictSpace} />
            <CommandDesc>wrong words can&apos;t be skipped</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay free backspace restore previous word toggle on off"
          ...
          >
            <IconArrowBackUp /> free backspace <ToggleBadge on={activeSet.freeBackspace} />
            <CommandDesc>restore previous word on backspace</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay hide live stats blank wpm acc while running toggle on off"
          ...
          >
            <IconEyeOff /> hide live stats <ToggleBadge on={activeSet.hideLiveStats} />
            <CommandDesc>blank wpm/acc while running</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay hide progress time word count bar toggle on off"
          ...
          >
            <IconEyeOff /> hide progress <ToggleBadge on={activeSet.hideProgress} />
            <CommandDesc>hide time/word count and progress bar</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay punctuation add commas periods exclamation marks toggle on off"
          ...
          >
            <IconAt /> punctuation <ToggleBadge on={activeSet.punctuation} />
            <CommandDesc>add punctuation marks to words</CommandDesc>
          </CommandItem>
          <CommandItem
            value="gameplay numbers add digits 0 1 2 3 4 5 toggle on off"
          ...
          >
            <IconHash /> numbers <ToggleBadge on={activeSet.numbers} />
            <CommandDesc>add numbers to words</CommandDesc>
          </CommandItem>
        </CommandGroup>
        ...
        <CommandGroup heading="appearance">
          ...
          >
            <IconKeyboard /> virtual keyboard <ToggleBadge on={activeSet.showKeyboard} />
            <CommandDesc>show key highlighter</CommandDesc>
          </CommandItem>
          <CommandItem
            value="appearance smooth caret animate caret movement toggle on off"
          ...
          >
            <IconPointer /> smooth caret <ToggleBadge on={activeSet.smoothCaret} />
            <CommandDesc>animate caret movement</CommandDesc>
          </CommandItem>
        </CommandGroup>
      </CommandList>
  ...
  return (
    ...
    >
      <Avatar className="size-5">
        ...
        <AvatarFallback className="rounded-full text-[9px] uppercase">
          {u.username.slice(0, 2)}
        </AvatarFallback>
      </Avatar>
      <span>{u.username}</span>
      <CommandDesc>
        <IconArrowRight className="size-3" /> view profile
      </CommandDesc>
    </CommandItem>
  );
});
...
function CommandDesc({ children }: { children: React.ReactNode }) {
  return (
    <span className="text-muted-foreground ml-auto text-[10px] shrink-0 flex items-center gap-1">
      {children}
    </span>
  );
}
...
function ToggleBadge({ on }: { on: boolean }) {
  return (
    <Badge
      ...
      className="ml-1 h-4 px-1.5 py-0 text-[10px] font-semibold uppercase"
    >
      {on ? "on" : "off"}
    </Badge>
  );
