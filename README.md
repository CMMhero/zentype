...
A full-stack typing test built on **Next.js**. Inspired by monkeytype. 80+ themes, 50+ fonts, cloud-synced stats, XP/levels, 110+ achievements, streak heatmaps, and global leaderboards.
...
- punctuation and numbers modes -- adds commas, periods, digits to the word pool
...
- Trophy UI (`achievement-grid` for cards, `achievement-list` for the see-all dialog): unlocked shows primary color + check, locked is muted + grayscale
...
Runs **fully without backend keys**. Guest mode, local results, english prompts, Postgres-fallback leaderboards all work out of the box. Add keys to unlock sync, OAuth, and Redis leaderboards.
